...
export const processJobBatch = internalAction({
  ...
  handler: async (ctx, args) => {
    const job = await ctx.runQuery(internalApi.userBookTextJobs.getJobInternal, { jobId: args.jobId });
    if (!job) return { ok: true };
    if (job.status !== "running") return { ok: true };

    const apiKey = process.env.OPENROUTER_API_KEY;
    if (!apiKey) {
      await ctx.runMutation(internalApi.userBookTextJobs.setJobPausedInternal, {
        jobId: args.jobId,
    ...
    for (let processed = 0; processed < batchSize; processed++) {
      if (nextPage > job.totalPages) {
        await ctx.runMutation(internalApi.userBookTextJobs.setJobCompletedInternal, { jobId: args.jobId });
        return { ok: true };
      }

      const exists = await ctx.runQuery(internalApi.userBookText.isPageExtractedInternal, {
        bookId: job.bookId,
        pageNumber: nextPage,
      });
      ...
      const prompt = `You are reading a single page of a personal study PDF.
...
Be thorough.`;
      ...
      await ctx.runMutation(internalApi.userBookTextJobs.setJobModelInfoInternal, {
        jobId: args.jobId,
      ...
      await ctx.runMutation(internalApi.userBookText.upsertUserBookPageTextInternal, {
        bookId: job.bookId,
        pageNumber: nextPage,
        content: cleaned,
        model: result.model,
    ...
    await ctx.runMutation(internalApi.userBookTextJobs.setJobProgressInternal, {
      jobId: args.jobId,
